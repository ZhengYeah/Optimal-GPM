...
class MinL1CircleMechanism(MinErrorMechanism):
    def __init__(self, endpoint_a, endpoint_b, epsilon, total_piece, probabilities=None):
        """
        :param endpoint_a: left endpoint of the bound
        :param endpoint_b: right endpoint of the bound
    ...
    def solve_probabilities(self):
        ...
        # worst-case x
        worst_x = (self.endpoint_a + self.endpoint_b) / 2
        m_1.addConstr(l[mid] <= worst_x, name="cons_3")
        m_1.addConstr(worst_x <= l[mid + 1], name="cons_3")
        ...
        # encoding proved correct
        left_distance = m_1.addVar(lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name="obj")
        m_1.addConstr(left_distance == sum(
            worst_x * p[i] * (l[i + 1] - l[i]) - obj_tmp[i] / 2 * p[i] for i in range(mid)), name="cons_4")
        right_distance = m_1.addVar(lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name="obj")
        m_1.addConstr(right_distance == sum(obj_tmp[i] / 2 * p[i] - worst_x * p[i] * (l[i + 1] - l[i]) for i in
                                            range(mid + 1, self.total_piece)), name="cons_4")

        m_1.setObjective(left_distance + right_distance + worst_x * worst_x * p[mid] - worst_x * p[mid] * l[mid] -
                         p[mid] * worst_x * l[mid + 1] + obj_center / 2 * p[mid], GRB.MINIMIZE)
