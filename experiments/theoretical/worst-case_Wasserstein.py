...
for i, _ in enumerate(epsilon):
    ...
    distance_SW[i] = wasserstein_distance(0, 1, 3, p, l, 0)
    # PM
    p, l = PM_on_01(epsilon[i], 0)
    distance_PM[i] = wasserstein_distance(0, 1, 3, p, l, 0)
    # optimal
    opt_PM = MinWassersteinMechanism(endpoint_a=0, endpoint_b=1, epsilon=epsilon[i], total_piece=3)
